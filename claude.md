...
•	HorizontalAxis.ItemPlacer: use HorizontalAxis.ItemPlacer.aligned(spacing = N) to place ticks and label slots every N x-units. Pass as itemPlacer = remember { HorizontalAxis.ItemPlacer.aligned(spacing = N) } to rememberBottom(). The formatter still controls what text appears — return "" to draw a tick with no label. For explicit x-values use series(x = listOf(...), y = listOf(...)); both params accept Collection<Number>.
